-- Add organization_admin to user_role enum if not already added
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'user_role') THEN
        -- Skip if the type doesn't exist yet
        RAISE NOTICE 'user_role type does not exist yet, skipping enum value addition';
    ELSIF NOT EXISTS (
        SELECT 1
        FROM pg_enum e
...
BEGIN
    IF EXISTS (
        ...
        AND policyname = 'Organization admins can view users in their organization'
    ) THEN
        DROP POLICY "Organization admins can view users in their organization" ON users;
    END IF;
END$$;
...
VALUES 
  (gen_random_uuid(), 'zach@wecr8.info', 'Zach Admin', 'admin', true),
  (gen_random_uuid(), 'admin@wecr8.info', 'Admin User', 'admin', true)
