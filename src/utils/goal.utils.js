...
export function calculateOnTimePercentage(jobs) {
    const completed = jobs.filter(j => j.status === 'completed');
    if (completed.length === 0)
        return 0;
    const onTime = completed.filter(j => j.completedDate && j.dueDate && j.completedDate <= j.dueDate);
    return (onTime.length / completed.length) * 100;
}
// Calculate throughput (jobs completed per day)
export function calculateThroughput(jobs, startDate, endDate) {
...
export function evaluateGoal(goal, jobs) {
...
export function suggestJobReordering(jobs) {
