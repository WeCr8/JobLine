...
export function importPart(row: Record<string, any>): Part {
  return {
...
export function importOperation(row: Record<string, any>): Operation {
  return {
    ...
    description: row.description,
    status: (row.status || row.Status || 'not_started') as Operation['status'],
...
export function importResource(row: Record<string, any>): Resource {
  return {
    id: String(row.id || row.resource_id || row.ResourceID),
    name: String(row.name || row.resource_name || row.ResourceName),
    type: (row.type || row.Type || 'machine') as Resource['type'],
...
export function importCapacity(row: Record<string, any>): Capacity {
  return {
...
export function importConstraint(row: Record<string, any>): Constraint {
  return {
    id: String(row.id || row.constraint_id || row.ConstraintID),
    type: (row.type || row.Type || 'custom') as Constraint['type'],
    description: row.description,
