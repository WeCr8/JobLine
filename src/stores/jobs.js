...
export const useJobsStore = defineStore('jobs', () => {
    ...
    const fetchJobs = async () => {
        ...
        try {
            ...
            if (import.meta.env.VITE_DEMO_MODE === 'true' && authStore.user?.email?.includes('demo')) {
                ...
                jobs.value = demoService.getDemoData('jobs');
            }
            else {
    ...
    const updateJobProgress = async (jobId, completedQuantity) => {
        loading.value = true;
        error.value = null;
        try {
            // Check if we're in demo mode
    ...
    // Computed properties
    const activeJobs = computed(() => jobs.value.filter(job => ['running', 'setup', 'on-hold'].includes(job.status)));
    const completedJobs = computed(() => jobs.value.filter(job => job.status === 'completed'));
    const urgentJobs = computed(() => jobs.value.filter(job => job.priority === 'urgent' && job.status !== 'completed'));
