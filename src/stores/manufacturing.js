...
export const useManufacturingStore = defineStore('manufacturing', () => {
    ...
    const digitalTwinCompliance = ref([]);
    const toolDataIntegrity = ref([]);
    const materialInventory = ref([]);
    const purchaseOrders = ref([]);
    const shippingReceiving = ref([]);
    const programmingTasks = ref([]);
    ...
    const fetchDepartments = async () => {
        ...
        try {
            ...
            if (import.meta.env.VITE_DEMO_MODE === 'true' && authStore.user?.email?.includes('demo')) {
                ...
                departments.value = demoService.getDemoData('departments');
            }
            else {
    ...
    const fetchMachines = async () => {
        ...
        try {
            ...
            if (import.meta.env.VITE_DEMO_MODE === 'true' && authStore.user?.email?.includes('demo')) {
                // Use hardcoded demo data
                machines.value = demoService.getDemoData('machines');
            }
            else {
                // Use real data from API
                const { data, error } = await supabase
    ...
    const departmentsByType = computed(() => {
        const production = departments.value.filter(d => ['cnc-machining', 'sheet-metal', 'welding', 'additive', 'assembly'].includes(d.id));
        const support = departments.value.filter(d => ['purchasing', 'shipping-receiving', 'material-stores', 'tdit', 'digital-twin-compliance', 'programming', 'quality', 'maintenance', 'planning'].includes(d.id));
    ...
    const activeMachines = computed(() => machines.value.filter(machine => machine.status === 'running'));
    const availableMachines = computed(() => machines.value.filter(machine => machine.status === 'idle'));
