...
export const useOptimizationStore = defineStore('optimization', () => {
    const partSimilarities = ref([]);
    const setupOptimizations = ref([]);
    const productionTrends = ref([]);
    const voiceNotes = ref([]);
    const mediaUploads = ref([]);
    const qualityInsights = ref([]);
    const engineeringAlerts = ref([]);
    const loading = ref(false);
    // Mock data for demonstration
    const mockPartSimilarities = [
        {
            id: '1',
            partNumber: 'PN-12345',
    ...
    const analyzePartSimilarity = async (partNumber) => {
        // Simulate AI analysis
        await new Promise(resolve => setTimeout(resolve, 2000));
        return partSimilarities.value.filter(p => p.partNumber !== partNumber);
    };
    const generateSetupOptimization = async (jobId) => {
    ...
    const addMediaUpload = async (upload) => {
        const newUpload = {
            ...upload,
            id: Date.now().toString(),
            createdAt: new Date().toISOString()
    ...
    const totalTimeSavings = computed(() => setupOptimizations.value.reduce((total, opt) => total + opt.timeSavings, 0));
    const totalCostSavings = computed(() => setupOptimizations.value.reduce((total, opt) => total + opt.estimatedROI, 0));
    const criticalQualityIssues = computed(() => qualityInsights.value.filter(insight => insight.impact === 'high'));
    const openEngineeringAlerts = computed(() => engineeringAlerts.value.filter(alert => alert.status === 'open'));
    return {
        partSimilarities,
        setupOptimizations,
        productionTrends,
