...
import { useJobsStore } from './jobs.ts';
...
export const useChatStore = defineStore('chat', () => {
  ...
  const addMessage = (content: string, isUser: boolean, type: 'text' | 'voice' | 'image' | 'part-lookup' = 'text', metadata?: any) => {
  ...
  const processUserQuery = async (query: string, inputType: 'text' | 'voice' = 'text') => {
  ...
  const processVoiceMessage = async (audioBlob: Blob): Promise<void> => {
    isProcessingVoice.value = true;
  ...
  const processImageForPartLookup = async (imageFile: File): Promise<PartMatch[]> => {
    ...
    try {
      ...
      const response = `I found ${mockMatches.length} similar parts in our system:\n\n` +
        mockMatches.map(match => 
          ...
          `• Current Jobs: ${match.currentJobs && match.currentJobs.length > 0 ? match.currentJobs.join(', ') : 'None'}\n` +
  ...
  const generateAIResponse = (query: string): { content: string; jobReferences?: string[] } => {
  ...
  const shouldTriggerIntegration = (message: ChatMessage): boolean => {
  ...
  const sendToIntegrations = async (message: ChatMessage) => {
  ...
  const sendToIntegration = async (integration: IntegrationConfig, message: ChatMessage) => {
    const payload = {
